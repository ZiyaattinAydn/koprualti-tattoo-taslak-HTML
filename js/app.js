...
document.querySelectorAll(".dd>button").forEach(b=>b.addEventListener("click",()=>{const isOpen=b.parentElement.classList.toggle("open");b.setAttribute("aria-expanded",String(isOpen));}));
...
const show=n=>{if(!pages.length)return;step=Math.max(0,Math.min(n,pages.length-1));pages.forEach((p,i)=>p.classList.toggle("active",i===step));bars.forEach((b,i)=>b.classList.toggle("active",i<=step));const current=document.querySelector("[data-step-current]");if(current)current.textContent=String(step+1).padStart(2,"0");window.scrollTo({top:0,behavior:"smooth"})};
